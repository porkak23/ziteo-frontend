...
// OTP_PROVIDER ('whatsapp' | 'twilio' | 'firebase'), default 'firebase'.
//
...
// verify(): valida la prueba entregada por el cliente. Para 'whatsapp' y
//           'twilio' la prueba es el código de 6 dígitos; para 'firebase' es
//           el ID token.
//
// Para cambiar de proveedor: editar el secret OTP_PROVIDER. Cero cambios de
// código.
...
import { twilioOtpAdapter } from './otp-twilio-adapter.ts'
...
export interface OtpProvider {
  readonly name: 'whatsapp' | 'firebase' | 'twilio'
...
export function getOtpProvider(): OtpProvider {
  switch (Deno.env.get('OTP_PROVIDER')) {
    case 'whatsapp': return whatsappOtpAdapter
    case 'twilio': return twilioOtpAdapter
    default: return firebaseOtpAdapter
  }
}
